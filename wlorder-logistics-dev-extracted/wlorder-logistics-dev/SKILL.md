...
```
订单录入 → 调度分拣 → 整车(二维码→司机扫码) / 零单(文员补录) → 已发货 → 费用核算 → 看板
```
...
- `/` 订单管理：快捷录入 + 列表（`@ui.refreshable`）
- `/shipments` 发货调度：二维码弹窗 + 零单补录弹窗 + 台账表格
- `/dashboard` 数据看板：统计卡片 + 明细列表（待开发）
- `/finance` 费用核算：对外费 / 对内成本 / 利润（待开发）
...
4. **Dialog 同级定义**：NiceGUI 弹窗必须与表格在同一 `with` 层级，通过隐藏 `ui.label` 传递当前行 ID
5. **访问前缀可配**：司机端二维码的 base_url 通过 Input 框动态配置，支持局域网 IP 或内网穿透域名

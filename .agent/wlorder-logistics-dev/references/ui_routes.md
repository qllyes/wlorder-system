...
### 订单明细页（detail_view）UX重构
- 顶部采用 `flat round` 返回图标按钮 + 标题 `订单明细：[订单编号]`。
- 表格上方新增上下文信息卡片（发货人/收货人/总件数/总运费）。
- 商品明细采用 `ui.table` 行内编辑（Quasar body slot + `q-input dense borderless`）。
- 右上角提供“💾 保存所有修改”批量提交，保存成功 `ui.notify(..., type='positive')`。
- 保存时进行数值防呆（非负校验、number输入）。

### 新增订单弹窗（新建发货单）模块化
- 模块 A：基础与物流信息（2列栅格，`outlined dense`）。
- 模块 B：财务与交接要求（`outlined dense`）。
- 模块 C：商品明细清单（导入后逐行编辑 + `➕ 新增商品行`）。
- 底部主操作：`🚀 确认创建订单`（主色按钮）。
